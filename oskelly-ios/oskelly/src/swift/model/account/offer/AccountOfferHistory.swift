//
//  AccountOfferHistory.swift
//
//  Created by Виталий Хлудеев on 09.02.18
//  Copyright (c) . All rights reserved.
//

import Foundation
import SwiftyJSON

public final class AccountOfferHistory: NSCoding {

  // MARK: Declaration for string constants to be used to decode and also serialize.
  private struct SerializationKeys {
    static let price = "price"
    static let text = "text"
    static let bold = "bold"
  }

  // MARK: Properties
  public var price: String?
  public var text: String?
  public var bold: Bool? = false

  // MARK: SwiftyJSON Initializers
  /// Initiates the instance based on the object.
  ///
  /// - parameter object: The object of either Dictionary or Array kind that was passed.
  /// - returns: An initialized instance of the class.
  public convenience init(object: Any) {
    self.init(json: JSON(object))
  }

  /// Initiates the instance based on the JSON that was passed.
  ///
  /// - parameter json: JSON object from SwiftyJSON.
  public required init(json: JSON) {
    price = json[SerializationKeys.price].string
    text = json[SerializationKeys.text].string
    bold = json[SerializationKeys.bold].boolValue
  }

  /// Generates description of the object in the form of a NSDictionary.
  ///
  /// - returns: A Key value pair containing all valid values in the object.
  public func dictionaryRepresentation() -> [String: Any] {
    var dictionary: [String: Any] = [:]
    if let value = price { dictionary[SerializationKeys.price] = value }
    if let value = text { dictionary[SerializationKeys.text] = value }
    dictionary[SerializationKeys.bold] = bold
    return dictionary
  }

  // MARK: NSCoding Protocol
  required public init(coder aDecoder: NSCoder) {
    self.price = aDecoder.decodeObject(forKey: SerializationKeys.price) as? String
    self.text = aDecoder.decodeObject(forKey: SerializationKeys.text) as? String
    self.bold = aDecoder.decodeBool(forKey: SerializationKeys.bold)
  }

  public func encode(with aCoder: NSCoder) {
    aCoder.encode(price, forKey: SerializationKeys.price)
    aCoder.encode(text, forKey: SerializationKeys.text)
    aCoder.encode(bold, forKey: SerializationKeys.bold)
  }

}
